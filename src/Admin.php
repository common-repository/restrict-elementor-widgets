...
class Admin extends Base {
	...
	public function dashboard_widget() {
		wp_add_dashboard_widget( 'cx-overview', __( 'Latest From Our Blog', 'restrict-elementor-widgets' ), [ $this, 'callback_dashboard_widget' ] );
	...
	public function callback_dashboard_widget() {
		...
		$_links = apply_filters( 'cx-overview_links', [
			'products'	=> [
				...
				'label'		=> __( 'Our Blog', 'restrict-elementor-widgets' ),
				'target'	=> '_blank',
			],
			'hire'	=> [
				'url'		=> add_query_arg( $utm, 'https://codexpert.io/hire/' ),
				'label'		=> __( 'Hire Us', 'restrict-elementor-widgets' ),
				'target'	=> '_blank',
			],
		] );
		...
		foreach ( $_links as $id => $link ) {
			$_has_icon = ( $link['target'] == '_blank' ) ? '<span class="screen-reader-text">' . __( '(opens in a new tab)', 'restrict-elementor-widgets' ) . '</span><span aria-hidden="true" class="dashicons dashicons-external"></span>' : '';
	...
	public function footer_text( $text ) {
		...
		// Translators: %1$s: plugin name, %2$s: review URL, %3$s: star rating.
	...
	public function register_controls( $controls_manager ) {
		...
		$controls_manager->register( new Control_Time() );
	}
